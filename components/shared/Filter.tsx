...
const Filter = ({ maxPrice, minPrice, maxMin, checkParams, category, counts }: Props) => {
  ...
  return (
    <>
    <Button ref={filterButtonRef} onClick={(e)=>toggleOverflow(e)} className="fixed duration-300 left-0 top-36 rounded-none md:hidden transition-all"><i className="fa fa-filter pointer-events-none"></i></Button>
    <div ref={divRef} className='transition-all duration-300 w-[25%] border-[1.5px] shadow-small px-5 max-[1023px]:w-[30%] max-[850px]:w-[35%] max-[1080px]:px-3 max-[880px]:px-2 max-md:w-[300px] max-md:rounded-l-none max-md:fixed max-md:bg-white  max-md:flex max-md:flex-col justify-center z-50 items-center max-md:overflow-y-scroll overflow-x-hidden max-md:h-full  max-md:translate-x-[-100%] max-[360px]:w-full max-[360px]:rounded-none top-0  left-0 ' >
      <div className='h-full max-md:w-[270px] py-10'>
          <div className="w-full h-fit flex justify-between"> 
            <h2 className='text-[28px]'>Фільтр</h2>
            <Button onClick={(e)=>toggleOverflow(e)} className="duration-300 size-12 md:hidden transition-all min-[361px]:hidden"><i className="fa fa-filter pointer-events-none"></i></Button>
          </div>
            
            <div className='mt-4 pb-4 w-full'>
              <Accordion type="single" collapsible className="w-full" defaultValue="item-1">
                <AccordionItem value="item-1">
                  <AccordionTrigger className="text-[18px] bg-zinc-100 font-medium py-[6px] px-3">Ціна</AccordionTrigger>
                    ...
                    <Slider
                      ...
                      <div className='flex gap-1 justify-between mt-7 w-full'>
                        <div>
                          ...
                          <input className='w-20 h-8 mt-2 text-center border flex items-center justify-center' onChange={(e) => setFilter({...filter, price: [e.target.value !== "₴" ? parseFloat(e.target.value.slice(1)) : 0, maxPrice]})} value={`₴${filter.price[0]}`} id="minPrice"/>
                        </div>
                        <div>
                          <label htmlFor="maxPrice">До</label>
                          <input className='w-20 h-8 mt-2 text-center border flex items-center justify-center' onChange={(e) => setFilter({...filter, price: [minPrice, e.target.value !== "₴" ? parseFloat(e.target.value.slice(1)): 0]})} value={`₴${filter.price[1]}`} id="maxPrice"/>
                        </div>
                      </div>
                  </AccordionContent>
                </AccordionItem>
            ...
            <div className='mt-4 pb-4 w-full min-[601px]:hidden'>
              <Accordion type="single" collapsible className="w-full">
                <AccordionItem value="item-1">
                  <AccordionTrigger className='text-[18px] bg-zinc-100 font-medium py-[6px] px-3'>Сортування</AccordionTrigger>
            ...
            {checkParamsNames.map((param) => (
              <div key={param} className='mt-4 pb-4 w-full'>
                <Accordion type="single" collapsible className="w-full">
                  <AccordionItem value="item-1">
                    <AccordionTrigger className='text-[18px] bg-zinc-100 font-medium py-[6px] px-3'>{checkParamsNamesUa[param]}</AccordionTrigger>
                    <AccordionContent className="pl-3 max-h-[300px] overflow-y-auto">
                      {checkParams[param].map((value, index)=>(
                        ...
                        <div className="flex items-center space-x-2 mt-4">
                          <Checkbox 
                           id={value} 
                           className="size-5 border-neutral-600 data-[state=checked]:bg-black data-[state=checked]:text-white"
          ...
          {params.map((param, index) => {
            ...
            return (
              <div className='mt-4 pb-4 w-full' key={index}>
                <Accordion type="single" collapsible className="w-full" defaultValue="item-1">
                  <AccordionItem value="item-1">
                    <AccordionTrigger className="text-[18px] bg-zinc-100 font-medium py-[6px] px-3">{paramsUa[param]}</AccordionTrigger>
                    <AccordionContent className="flex flex-col items-center shrink-0 px-3">
                      <Slider
...
const FilterInput = ({ paramName, setting, maxMin, filter, setFilter }: { paramName: ParamsName, setting: "min" | "max", maxMin: Props["maxMin"], filter: FilterType, setFilter: Dispatch<SetStateAction<FilterType>>}) => {
  ...
  return (
    <div>
      ...
      <input
        className="appearance-none w-20 h-8 mt-2 text-center border flex items-center justify-center"
