...
const LinkButton = ({ children, href, className, type, ...props }: Props) => {
  ...
  return (
    <TransitionLink href={href} {...props} type={"left"}>
      <motion.div
        className={cn(`relative w-fit h-fit flex justify-center items-center font-medium gap-2 border border-black px-4 py-2 overflow-hidden ${type === "white" ? "border-white bg-white" : "border-black"}`, className)}
      ...
      >
        ...
        <motion.div
          className="relative z-[5] flex items-center max-[330px]:hidden"
          initial={false}
          animate={{
